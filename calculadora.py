...
while True:
    ...
    if operacion.lower() == "b":
        number_1 = input(f"haz elegido: {operacion}, Cual es el first number: ")
        number_2 = input(f"ok, elegiste {number_1}, Cual es el second number: ")
        ecuacion = float(number_1) + float(number_2)
        ...
        try:
            number_1 = input(f"haz elegido: {operacion}, Cual es el numero: ")
            a = math.sqrt(float(number_1))
            print(f"el resultado de tu Raiz Cuadrada es: {a:.2f}")
        except:
            print(f"Los numeros negativos, como {number_1} no aplican para raiz cuadrada, ya que son numeros imaginarios")
            b = cmath.sqrt(float(number_1))
